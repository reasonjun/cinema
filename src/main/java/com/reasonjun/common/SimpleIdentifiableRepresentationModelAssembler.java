 ...
 */
public class SimpleIdentifiableRepresentationModelAssembler<T> implements
  ...
  /**
   * Default a assembler based on Spring MVC controller, resource type, and {@link
   * LinkRelationProvider}. With this combination of information, resources can be defined.
   *
  ...
  /**
   * Add single item self link based on the object and link back to aggregate root of the {@literal
   * T} domain type using {@link LinkRelationProvider#getCollectionResourceRelFor(Class)}}.
   *
   * @param resource
   */
  public void addLinks(EntityModel<T> resource) {
    ...
    resource.add(getCollectionLinkBuilder().withRel(
        this.relProvider.getCollectionResourceRelFor(this.resourceType)));
  }
  ...
  /**
   * Build up a URI for the collection using the Spring web controller followed by the resource type
   * transformed by the {@link LinkRelationProvider}. Assumption is that an {@literal
   * EmployeeController} serving up {@literal Employee} objects will be serving resources at {@code
   * /employees} and {@code /employees/1}. If this is not the case, simply override this method in
   * your concrete instance, or resort to overriding {@link #addLinks(EntityModel)} and {@link
   * #addLinks(CollectionModel)} where you have full control over exactly what links are put in the
   * individual and collection resources.
   *
   * @return
   */
  protected LinkBuilder getCollectionLinkBuilder() {
    ...
    for (String pathComponent : (getPrefix() + this.relProvider.getCollectionResourceRelFor(
        this.resourceType))
        .split("/")) {
